...
static void policy_check(const std::string &s) {
  if (s != "ignore" && s != "warn" && s != "error")
    throw NLP::Exception("unrecognised policy '" + s +
                         "' [ignore, warn, error]");
}

CandC::Config::Config(const OpPath *base, const std::string &name,
                      const std::string &desc)
    : Directory(name, desc, base),
      verbose(*this, "verbose", "verbose output", false),
      comments(*this, "comments", "comment the output", false),
      morph(*this, "morph",
            "directory containing the morphological analyser info",
            "//verbstem.list", &path),
      printer(*this, "printer",
              "parser printing style [deps, grs, prolog, boxer, ccgbank, xml, "
              "debug, js]",
              "grs"),
      decoder(*this, "decoder", "parser decoder [deps, derivs, random]",
              "derivs"),
      compact(*this, SPACE, "compact",
              "use compact output representation if it exists", false),
      maxwords(*this, SPACE, "maxwords",
               "maximum sentence length the parser will accept", 250),
      maxwords_policy(*this, "maxwords_policy",
                      "parser behaviour on sentences longer than maxwords "
                      "[ignore, warn, error]",
                      &policy_check, "ignore"),
      skip_quotes(*this, "skip_quotes",
                  "skip over all quote and double quote tokens", false),
      trans_brackets(*this, "trans_brackets",
                     "translate all brackets to Penn Treebank notation", false),
      pos(&path), chunk(&path), ner(&path), super(&path), parser(&path),
      pos_dict_cutoff(pos, "dict_cutoff", "POS tagger tag dictionary cutoff",
                      5),
      chunk_dict_cutoff(chunk, "dict_cutoff", "chunker tag dictionary cutoff",
                        5),
...
CandC::CandC(Config &cfg, const std::string &PREFACE)
    : cfg(cfg), PREFACE(PREFACE),
      // load taggers
      pos(cfg.pos), chunk(cfg.chunk), ner(cfg.ner),
      // load integrated supertagger and parser
      integration(cfg.integration, cfg.super, cfg.parser, sent),
      decoder(cfg.decoder()), pos_state(pos.create_state()),
      chunk_state(chunk.create_state()), ner_state(ner.create_state()) {
  morph_initialise(cfg.morph().c_str());
}

static bool is_quote(const std::string &s) {
  switch (s[0]) {
  case '`':
    return s == "``" || s == "`";
  case '\'':
...
static void ptb_bracket(std::string &s) {
  if (s.size() != 1)
    return;
...
void split_token(std::string token, std::string &word, std::string &pos,
                 std::string &super) {
  word = "";
  pos = "";
  super = "";
  ...
  for (string::iterator i = token.begin(); i != token.end(); ++i) {
    ...
    default:
      throw NLP::Exception(
          "input token should only have 2 pipes -- word|pos|super");
    }
  }
}

bool CandC::load_sentence(std::ostream &log) {
  sent.reset();
  ...
  if (sent.words.size() > cfg.maxwords()) {
    ...
    if (cfg.verbose() || cfg.maxwords_policy() == "warn")
      log << "ignoring long sentence (length " << sent.words.size()
          << " words) on line " << nlines << endl;
...
bool CandC::load_oracle(std::ostream &log) {
  sent.reset();
...
bool use_morpha(const std::string &word, const std::string &pos) {
...
double CandC::parse(IO::Input &in, IO::Output &out, IO::Log &log, bool START,
...
double CandC::oracle(IO::Input &in, IO::Input &constraints, IO::Output &out,
                     IO::Log &log, bool START,
                     const std::string &override_printer) {
  reset();
  ...
  while (read_buffer(in.stream)) {
    ...
    integration.parse(sent, decoder, printer,
                      false); // don't use the supertagger
  }
